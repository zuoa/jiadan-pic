...
async function generateApiCode() {
  try {
    ...
    const { files } = await generateApi({
      ...
      templates: path.resolve(process.cwd(), 'scripts/api-templates'),
    });
...
`;
    ...
    fs.writeFileSync(path.join(outputDir, 'index.ts'), indexContent.trim());
...
async function createBasicTemplate() {
  const basicApiContent = `
// 基础API模板 - 当OpenAPI规范不可用时使用
...
`;
  ...
  fs.writeFileSync(path.join(outputDir, 'api.ts'), basicApiContent.trim());
